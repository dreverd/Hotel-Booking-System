...
public abstract class BaseRestController {
	...
	protected <T> ResponseWrapper<List<T>> generateResponse(List<T> records) throws ResourceNotFoundException {
		if (records.size() == 0) {
	...
	protected <T,R> ResponseWrapper<List<R>> generateResponse(List<T> records, Function<T, R> mapper) throws ResourceNotFoundException {
		return generateResponse(convertToDto(records, mapper));
	}
